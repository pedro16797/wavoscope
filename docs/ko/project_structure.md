...
-   **`src/`**: 핵심 애플리케이션 로직 및 소스 코드를 포함합니다.
    ...
    -   **`cli/`**: 명령줄 인터페이스 유틸리티를 포함합니다.
        ...
        -   `gui.py`: 그래픽 사용자 인터페이스를 위한 `pywebview` 로직.
        -   `launcher.py`: 백엔드 및 프론트엔드 실행을 위한 헬퍼 로직.
    -   **`frontend/`**: React 기반 그래픽 사용자 인터페이스.
        ...
        -   `dist/`: 빌드된 프로덕션 자산.
        -   `tests/`: 프론트엔드 구성 요소 및 스토어 로직을 위한 단위 테스트.
    -   **`scripts/`**: 자동화 및 유틸리티 스크립트(예: 스크린샷 생성, 실행기 생성).
    -   **`session/`**: 프로젝트 지속성 및 고수준 상태를 처리합니다.
        -   `project.py`: 오디오, 메타데이터(플래그) 및 캐싱을 하나로 묶는 `Project` 클래스.
        -   `manager.py`: `.oscope` 사이드카 파일 I/O를 처리합니다.
        -   `flags.py`: 리듬 및 코드 플래그 리스트를 관리합니다.
        -   `undo.py`: 델타 기반 실행 취소 기록 관리.
    -   **`tests/`**: 백엔드 및 핵심 로직을 위한 단위 및 통합 테스트.
    -   **`utils/`**: 일반적인 헬퍼 함수 및 공유 유틸리티(로깅, 설정 등).
...
-   **`Wavoscope` / `Wavoscope.exe`**: 빌드된 실행기 실행 파일(스크립트로 생성됨).
