...
export const FlagDialog: React.FC<FlagDialogProps> = ({ idx, flag, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-surface border-[width:var(--ui-border)] border-grid rounded-[var(--ui-radius)] shadow-2xl w-full max-sm overflow-hidden text-text flex flex-col isolation-auto"
           onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b-[width:var(--ui-border)] border-grid font-bold text-sm uppercase tracking-widest opacity-80 flex justify-between items-center bg-surface">
            <span>Edit Flag</span>
        ...
        <div className="p-4 space-y-4">
            ...
            {hasNext && (
                <button onClick={handleInsertN} className="w-full py-2 rounded-[var(--ui-radius)] bg-white/5 hover:bg-white/10 text-xs font-bold transition-colors border-[width:var(--ui-border)] border-grid">
                    Insert N flags between this and next
                </button>
            )}
        </div>
        <div className="p-4 border-t-[width:var(--ui-border)] border-grid flex items-center justify-between gap-2 bg-surface">
